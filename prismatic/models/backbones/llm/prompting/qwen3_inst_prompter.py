...
class Qwen3ChatPromptBuilder(PromptBuilder):
    def __init__(self, model_family: str, system_prompt: Optional[str] = None) -> None:
        ...
        # Note =>> There's no BOS token in Qwen-3, only EOS token
        self.bos, self.eos = "<BOS>", "<|im_end|>"

        # Get role-specific "wrap" functions
        #   =>> Note that placement of <bos>/<eos> were based on experiments generating from Phi-2 in Chat mode
        self.wrap_human = lambda msg: f"<s><|user|>\n{msg}<|end|>\n<|Assistant|>\n"
